...
object Day7 {
  ...
  case class SimpleCard(label: String) extends Card {
    override def jValue: Int = 11
  }

  case class JokerCard(label: String) extends Card {
    override def jValue: Int = 1
    def isJoker: Boolean = label == "J"
  }

  trait Hand[C <: Card] extends Ordered[Hand[C]]{
    val cards: Seq[C]
    val bid: Int
  ...
  def task[C <: Card](hands: Seq[Hand[C]]): Int =
